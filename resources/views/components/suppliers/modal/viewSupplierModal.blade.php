        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @if($user->hasPermission(PermissionValue::GERER_FOURNISSEURS))
                    @if($edit)
                        <button  class="btn btn-secondary mb-2 btn-load-modal" id="returnViewSupplierButton" title="Retourner à l'affichage du fournisseur" type="button" data-url="{{ route('suppliers.modal.view-details', ['id' => $supplier->getId(), 'edit' => false]) }}">
                    ...
                    @else
                        <button  class="btn btn-secondary mb-2 btn-load-modal" id="editSupplierButton" title="Modifier le fournisseur" type="button" data-url="{{ route('suppliers.modal.view-details', ['id' => $supplier->getId(), 'edit' => true]) }}">
                ...
                <form id="editSupplier-{{$supplierId}}" class="ajax-form" method="POST" class="needs-validation" enctype="multipart/form-data" action="{{route('suppliers.modal.view-details', ['id' => $supplierId, 'edit' => $edit])}}" autocomplete="off">
                    @csrf
